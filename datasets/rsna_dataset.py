...
class RSNAVolumeDataset(Dataset):
    ...
    def __init__(
        ...
        # Find all segmentation files AND check alignment
        self.seg_files = []
        ...
        # Filter to keep only aligned segmentation files
        self.valid_seg_files = self._filter_aligned_samples(self.seg_files)

        # Replace original seg_files
        self.seg_files = self.valid_seg_files

        if num_samples is not None:  
            self.seg_files = self.seg_files[:num_samples]

        print(f"Found {len(self.seg_files)} aligned segmentation files")

    def _filter_aligned_samples(self, seg_files: List[str]) -> List[str]:
        """
        Filter segmentation files to only include those with corresponding images.  

        Returns:
            List of segmentation files that have both:  
            - .nii/. nii.gz file in segmentation_dir
            - Corresponding image directory in image_dir
        """
        aligned_files = []
        missing_images = []
        ...
        for seg_file in seg_files:  
            ...
            if has_images:
                aligned_files.append(seg_file)
            else:
                missing_images.append(study_id)

        # Print alignment statistics
        print(f"\nData Alignment Statistics:")
        ...
        if missing_images:
            print(f"\nWarning: study_id missing images:")
            for study_id in missing_images[: 10]:  # Show first 10 only
                print(f"  - {study_id}")
    ...
    def load_nifti_volume(self, seg_file: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if image_study_dir is None:
            # This case should not happen now, as we filter in __init__
            raise FileNotFoundError(
                f"Images not found for {study_id}. "
                f"This should not happen if data alignment filtering is working correctly."
            )
        else:
            # Load JPEG slices
            jpeg_files = sorted([
