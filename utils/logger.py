...
class TrainingLogger:
    ...
    def __init__(self, log_dir: str, config: dict, run_name: str = None, config_path: str = None):
        """
        Initialize logger.
    ...
    def log_metrics(self, epoch: int, phase: str, metrics: dict, lr: float = None):
        ...
        if 'loss_ce' in metrics or 'loss_l1' in metrics or 'loss_giou' in metrics:
            ...
            if 'loss_l1' in metrics:
                self.info(f"    loss_l1:    {metrics['loss_l1']:.4f}")
            if 'loss_giou' in metrics:
                self.info(f"    loss_giou:  {metrics['loss_giou']:.4f}")
