...
class RSNAVolumeDataset(Dataset):
    ...
    def __init__(
        ...
        target_width: int = 64,  # Used only when target_size is None (proportional resize)
        target_size: Optional[Tuple[int, int, int]] = None,  # Fixed (D, H, W); overrides target_width
        train: bool = True,
        augment: bool = False,
        ...
        Args:
            ...
            target_width: Target width (2nd dimension); used only when target_size is None.
                Other dimensions scale proportionally.
            target_size: Fixed (D, H, W) target size. When provided, overrides target_width
                and resizes every volume to exactly this shape.
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # Resize volume to target shape (D, H, W)
        if self.target_size is not None:
            # Fixed-size resize: every volume becomes exactly target_size
            target_size = self.target_size
        else:
            # Proportional resize based on target_width (2nd dimension)
            original_width = volume.shape[1]
